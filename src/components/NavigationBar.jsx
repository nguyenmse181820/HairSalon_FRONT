    ...
    <header>
      <div className="sticky-container fixed w-full z-10 bg-white">
          ...
          <div className="right-container w-1/2 flex justify-end space-x-5 font-montserrat text-sm">
            ...
            <div className="flex items-center">
              <FontAwesomeIcon
                ...
                icon={faUser}
              />
              <Link to="/account" className="hidden lg:inline-block">
                My Account
              </Link>
            </div>
            <div className="flex items-center">
        ...
        <div className="menubar">
          ...
          <ul className="hidden lg:flex justify-center items-center space-x-[7rem] list-none pt-2">
            {["SERVICES", "BOOKING", "PRODUCTS", "CONTACT", "ABOUT US"].map(
              (menuItem, i) => (
                <li
                  key={i}
                  className="nav-link font-semibold text-sm cursor-pointer"
                >
                  {menuItem === "CONTACT" ? (
                    ...
                    <Link to="/booking/service" onClick={toggleMenu}>
                      {menuItem}
                    </Link>
                  ) : (
                    menuItem
                ...
                (menuItem, i) => (
                  <li key={i} className="nav-link font-semibold cursor-pointer">
                    {menuItem === "ABOUT US" ? (
                      <Link to="/about-us" onClick={toggleMenu}>
                        {menuItem}
                      </Link> // Close the menu when clicked in mobile view
                    ) : (
                      menuItem
                    )}
                  </li>
